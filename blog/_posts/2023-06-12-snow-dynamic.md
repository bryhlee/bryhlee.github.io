...
*The different colors in this diagram represent different materials. The camera is positioned below the canvas (without FOV distortion) and captures all displacement. Note that the displacement for a cone, pictured here, is not based on the cone's object, but the cross section of the points that make contact with the snow.*

![Render Target Camera Perspective](/assets/img/snow-dynamic/camera_pov.png)
*The camera's perspective. A cone is painted to the canvas as a circle, which is the cone's base.*
...
Right now, our landscape looks like a black canvas with mask channels layered on top of each other. Now that we have a texutre which abstractly represents our material, we must create the snow landscape, which will be float above our base landscape mesh.
...
Our final landscape material is assigned to the Virtual Heightfield Mesh, which handles our tessleation and LOD calculations for us. All we need to do is provide world displacement by sampleing our mask texture from our base landscape.

![Final Material](/assets/img/snow-dynamic/graph.png)
*Our Virutal Heightfield Mesh final landscape material.*

First, we sythensize several Megascan materials for our Fresh, Rough, and Ground snow, utilizing our RGB mask to determine which material to render. You'll notice that our fresh material references some noise textures to add flavor to our landscape, which is a quick hack to avoid execessive tiling in RnD projects.

Next, we have to calcualte our normals. This was rather complicated, since I orignally generated normals from our heightmap on our mask material and passed them through to our Virutal Height Field mesh via an RVT refernce, but this ended up crashing my entire system and corrupting my material (hence the system restore I referenced earlier). So, I decided to completely rewrite this portion, and calculate normals on our VHM manually. This must be blended with normals generated from our height map, which required some manual (and tedious) conversion.
...
# Improvements
Overall, I'm satisified I was able to create a somewhat appealing snow trail system and prove that it can work with high quality texutres. There's some areas that could be improved upon, especially when it comes to styilization and making the system more reslient (such as using more referenced values to avoid outdated hardcoded numerics).

For this project, enhancing the performance of our material masks is a key area for improvement. The current use of Spiral Blur is proving to be too resource-intensive, especially for frequent displacement. A more effective approach could be investigating more realistic snow dynamics, particularly the possibility of animating how snow crumbles, to see if it's a viable option. My extensive research into Material Point Methods (MPMs) for snow simulation, as detailed in (this Disney Animation research paper)[https://media.disneyanimation.com/uploads/production/publication_asset/94/asset/SSCTS13_2.pdf], reveals that while MPMs are too demanding for real-time use, we might be able to create a dynamic system inspired by a simplified version of this concept. This would involve a focus on how snow breaks along seams, moving our caclulations and snow dynamics away from pure-shader and material expressions to something that would be based on a combination of mesh deformation and FXs.

Additionally, I'm considering moving away from using the Virtual Heightfield Mesh due to several encountered issues, such as GPU memory constraints and unclear LOD (Level of Detail) calculations. With the introduction of a Displacement attribute in Unreal Engine 5.2 for base materials, we might be able to bypass the need for VHM altogether.